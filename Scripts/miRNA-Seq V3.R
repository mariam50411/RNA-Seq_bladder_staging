...
library(DESeq2)
data.path="./mirnadata"
files <- list.files(path = data.path, recursive = T, pattern = "quantification.txt")
...
for(i in 2: length(files))
{
  ...
  mirna.exp.tmp = temp[,1:2]
  rownames(mirna.exp.tmp) = mirna.exp.tmp[,1]
  mirna.exp.tmp = mirna.exp.tmp[-1]
  colnames(mirna.exp.tmp) = file.id
  mirna.exp=cbind(mirna.exp,mirna.exp.tmp)
  
}

rown = as.data.frame(rownames(mirna.exp))


# check duplciation of of mirna IDs?  
x=duplicated(rown)  
sum(x)
...
# mirna.exp= aggregate(mrna.exp.data, list(mrna.exp2$symbol),FUN=mean)
# 
# rownames(mirna.exp)=mirna.exp$Group.1
# mirna.exp=mirna.exp[-1]
# 
...
###### load the mirna sample sheets

pheno <- read.delim("./mirnadata/gdc_sample_sheet.2018-12-21.tsv")
View(pheno)
...
# names(mirna.exp)= pheno$Sample.ID
...
stageI.exp=mirna.exp[, names(mirna.exp)%in% stageI.samples]
stageII.exp=mirna.exp[, names(mirna.exp)%in% stageII.samples]
stageIII.exp=mirna.exp[, names(mirna.exp)%in% stageIII.samples]
stageIV.exp=mirna.exp[, names(mirna.exp)%in% stageIV.samples]
...
save(mirna.exp,pheno, exp.sub,pheno.sub ,file="miRNA-seq_Bladder.RDATA")
...
write.table(d1_2, "mirna_DEMs1_2.txt", sep = "\t", col.names = FALSE)
...
write.table(d2_3, "mirna_DEMs2_3.txt",  sep = "\t", col.names = FALSE)
...
write.table(d3_4, "mirna_DEMs3_4.txt", sep = "\t", col.names = FALSE)
...
write.table(d4_5, "mirna_DEMs4_5.txt",  sep = "\t", col.names = FALSE)



pheno$Type.Stage = ifelse(pheno$Sample.ID %in% c("TCGA-C4-A0F1-01A", "TCGA-BL-A0C8-01A", "TCGA-C4-A0F7-01A", "TCGA-C4-A0F1-01A", "TCGA-BL-A13J-01A", "TCGA-BL-A13I-01A", "TCGA-BL-A13J-01A", "TCGA-BL-A0C8-01A", "TCGA-C4-A0F7-01A"), pheno$Type.Stage <- "noooo", ifelse(pheno$Sample.Type == "Solid Tissue Normal", pheno$Type.Stage <- "Solid Tissue Normal", paste(pheno$Sample.Type, pheno$stage) ) )
par(mfrow=c(1,1))
with(res.degs1_2, plot(log2FoldChange, -log10(pvalue), pch=20, main="Volcano plot", xlim=c(-3,3)))
